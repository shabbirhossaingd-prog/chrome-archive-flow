  ...
  head: () => ({
    meta: [
      { title: "Enter the Dark — Contact ZZERKOFF" },
      {
        name: "description",
        content:
          "Questions, support or collaborations. Reach ZZERKOFF on WhatsApp, Instagram or email.",
      },
      { property: "og:title", content: "Enter the Dark — Contact ZZERKOFF" },
      { property: "og:description", content: "Questions, support or collaborations." },
    ],
  }),
...
function ContactPage() {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    window.open(site.wa(text), "_blank", "noopener");

    toast("Message ready", {
      description: "WhatsApp opened with your message.",
    });
  ...
  const cards = [
    {
      title: "WHATSAPP",
      sub: "SUPPORT / QUESTIONS",
      cta: "MESSAGE US →",
      href: site.wa(`Hi ${site.brand}, I have a question.`),
    },
    {
      title: "INSTAGRAM",
        ...
        <div className="mx-auto max-w-6xl">
          <Reveal>
            <PageHeading
              ...
              sub="Questions, support or collaborations."
            />
          </Reveal>

          <div className="mt-16 grid gap-4 sm:gap-6 lg:grid-cols-3">
            {cards.map((card, index) => (
              <Reveal key={card.title} delay={index * 130}>
      ...
      <section className="px-5 py-28 sm:px-8 sm:py-36">
        <div className="mx-auto max-w-2xl">
          <Reveal>
            <form
              onSubmit={submit}
              className="glass-panel space-y-6 rounded-[28px] p-7 sm:p-10"
            >
              <div>
