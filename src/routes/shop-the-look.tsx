  ...
  head: () => ({
    meta: [
      { title: "Shop the Look — ZZERKOFF" },
      {
        name: "description",
        content:
          "Curated ZZERKOFF looks built from rings, chains, bracelets and metal objects.",
      },
    ],
  }),
        ...
        <div className="mx-auto max-w-7xl">
          ...
          {looksQuery.error ? (
            <p className="py-20 text-center text-[10px] uppercase tracking-[0.4em] text-muted-foreground">
              Looks could not load. Try refreshing.
            </p>
          ) : looks.length === 0 && !isLoading && !looksQuery.isLoading ? (
            <p className="py-20 text-center text-[10px] uppercase tracking-[0.4em] text-muted-foreground">
              No published looks yet.
            </p>
          ) : (
            <div className="mt-12 space-y-20">
              {looks.map(({ look, items }: any, lookIndex: number) => (
                <section key={look.id}>
