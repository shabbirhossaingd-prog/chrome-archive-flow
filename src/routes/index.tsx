...
const BRAND_NAME = "ZZERKOFF";
const OG_IMAGE = `${SITE_URL}/images/zzerkoff-logo.png`;
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: HOME_SOCIAL_DESCRIPTION },
      ...
      { property: "og:image:alt", content: "ZZERKOFF chrome accessories" },
      { name: "twitter:card", content: "summary_large_image" },
      { name: "twitter:title", content: BRAND_NAME },
      { name: "twitter:description", content: HOME_SOCIAL_DESCRIPTION },
      { name: "twitter:image", content: OG_IMAGE },
      { name: "twitter:image:alt", content: "ZZERKOFF chrome accessories" },
    ],
    links: [
...
function Index() {
  ...
  const showDrop =
    ...
    (productsLoading || newDrop.length > 0);

  const showFeatured =
    (home.show_featured ?? true) && !productsLoading && !!featured;
  ...
  const hasDropTarget = productsLoading || newDrop.length > 0;
  ...
  const heroCtaHref =
    ...
    (hasDropTarget ? "/collection" : "/shop");

  const statementTitle = (
    home.statement_title ?? "NOT MADE\nTO BLEND IN."
  ).split("\n");
  ...
  const customSections = (home.sections ?? []).filter(
  ...
  return (
    <div className="relative min-h-screen overflow-x-clip bg-background">
      ...
      <section className="relative isolate flex min-h-screen flex-col items-center justify-center overflow-hidden px-5 text-center">
        ...
        <Reveal immediate className="mt-2">
        ...
        <Reveal immediate className="mt-12">
          <a
            href={heroCtaHref}
        ...
        >
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="home-mobile-product-limit mt-14">
              <ProductGrid products={newDrop} loading={productsLoading} />
            </div>
          </div>
        </section>
      )}
      ...
      {customSections.map((section, index) => (
        ...
        >
          ...
          >
            ...
            <Reveal delay={section.image ? 60 : 0}>
              <SectionLabel>
            ...
            <div className="mt-10 space-y-6 font-editorial text-lg leading-relaxed text-muted-foreground">
              {aboutBody.map((paragraph, index) => (
                <p
                  key={`${paragraph}-${index}`}
                  className={
                    index === aboutBody.length - 1 ? "text-chrome" : ""
                  }
                >
                  {paragraph}
                </p>
